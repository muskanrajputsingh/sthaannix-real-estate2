...
const UsersOverview = ({ users, stats, usersLoading, statsLoading }) => {
  ...
  return (
    <>
      ...
      {/* Filter + Table header remains the same */}
      <div className="my-6 flex flex-col gap-2 sm:flex-row sm:justify-between sm:items-center">
        <h2 className="text-lg font-semibold text-gray-700">Users Overview</h2>
        <select
          className="border rounded-lg px-3 py-2 text-sm"
          value={roleFilter}
      ...
      <div className="overflow-x-auto bg-white rounded-xl shadow border mb-6 hidden md:block">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
      ...
      {/* Mobile card version: visible below md */}
      <div className="block md:hidden space-y-4 mb-6">
        {paginatedUsers.length === 0 ? (
          <div className="text-center py-6 text-gray-500 bg-white rounded-xl shadow border">
            No users found
          </div>
        ) : (
          paginatedUsers.map((user, idx) => (
            <div key={user._id || idx} className="bg-white rounded-xl shadow border p-4 space-y-2">
              <div className="flex justify-between">
                <span className="text-gray-800 font-semibold">{user.name || "-"}</span>
                <span className="text-xs text-blue-700 font-bold">{user.role || "-"}</span>
              </div>
              ...
              <div className="flex items-center text-xs text-gray-500 gap-2">
                <Home className="w-4 h-4 text-gray-400" />
                Properties: {user.totalProperties || 0}
              </div>
              <div className="flex items-center text-xs text-gray-500 gap-2">
                <ClipboardList className="w-4 h-4 text-gray-400" />
